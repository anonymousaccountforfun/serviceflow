...
import { asyncHandler, sendSuccess, sendError, errors } from '../utils/api-response';
 ...
 * }
 */
router.post('/weekly-report', verifyCronSecret, asyncHandler(async (req, res) => {
    ...
    try {
      ...
      for (const user of org.users) {
        ...
        if (result.success) {
          ...
          failed++;
          errorList.push(`${org.id}:${user.email} - ${result.error?.message}`);
        }
      }
    } catch (orgError: any) {
      ...
      failed++;
      errorList.push(`${org.id} - ${orgError.message}`);
    }
  }
  ...
  sendSuccess(res, {
    ...
    failed,
    periodLabel,
    errors: errorList.length > 0 ? errorList.slice(0, 10) : undefined, // Limit error details
  });
}));
 ...
 */
router.post('/send-test-report', verifyCronSecret, asyncHandler(async (req, res) => {
  // Only allow in development
  if (process.env.NODE_ENV === 'production') {
    return errors.forbidden(res, 'Test endpoint not available in production');
  }

  const { email: recipientEmail, organizationId } = req.body;

  if (!recipientEmail || !organizationId) {
    return errors.validation(res, 'email and organizationId required');
  }
  ...
  if (result.success) {
    sendSuccess(res, { emailId: result.emailId, reportData });
  } else {
    return sendError(res, 'SEND_FAILED', result.error?.message || 'Failed to send email', 500);
  }
}));

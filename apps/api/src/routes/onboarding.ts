 ...
 */
router.post('/seed-sample-data', asyncHandler(async (req, res) => {
  ...
  if (existingJobs > 0) {
    return errors.validation(res, 'Organization already has data. Sample data can only be added to empty accounts.');
  }
 ...
 */
router.delete('/sample-data', asyncHandler(async (req, res) => {
  const orgId = req.auth!.organizationId;
  ...
  if (!settings.sampleDataSeeded) {
    return errors.validation(res, 'No sample data to remove.');
  }
...
router.post('/complete', asyncHandler(async (req, res) => {
  ...
  if (!org || !user) {
    return errors.notFound(res, 'Organization or user');
  }
